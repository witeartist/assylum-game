...
function problems(L: LevelData, keyCount: number): string[] {
  ...
  const closed = WalkGrid.fromRows(L.rows).withSolid(L.lockedDoors.flatMap(d => d.doorTiles));
  ...
  const open = distanceMap(WalkGrid.fromRows(L.rows), L.playerSpawn);
...
describe("level generator", () => {
  for (const diff of Object.values(DIFFICULTIES)) {
    it(`always produces winnable levels (${diff.id})`, () => {
      for (let seed = 1; seed <= 150; seed++) expect(problems(generateLevel(seed * 7919, diff.keyCount), diff.keyCount), `seed ${seed * 7919}`).toEqual([]);
    });
    it(`fallback level is winnable (${diff.id})`, () => {
      for (let seed = 1; seed <= 20; seed++) expect(problems(generateFallbackLevel(seed, diff.keyCount), diff.keyCount)).toEqual([]);
    });
  }
