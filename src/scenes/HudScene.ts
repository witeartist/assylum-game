...
export class HudScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    label(this, m, CANVAS_H - 8, CONTROLS_HINT[runner ? "runner" : "hunter"], "tiny", INK.hint, { origin: [0, 1] });
    ...
    this.stamina = new Meter(this, m, CANVAS_H - 50, "ui/icon_stamina", 110, "good").setVisible(runner);
  ...
  update(): void {
    ...
    if (w.director.bossSpawned) {
      ...
      this.boss.setText("Желочь через " + s + "с").setColor(s < 30 ? TONES.bad.ink : TONES.warn.ink);
    }
    hstack([this.keys, this.fuses, this.runners, this.boss], LAYOUT.margin, LAYOUT.gap);
    ...
    this.stamina.setVisible(body);
    this.battery.setVisible(body);
    this.slots.forEach(sl => sl.setVisible(body));
    if (body) {
      this.stamina.set(a.stamina / a.staminaMax, a.exhausted ? "bad" : a.adrenaline > 0 ? "key" : "good");
      ...
      this.name.setText("▶ " + a.def.name + (carrying ? "  [предохранитель]" : ""));
    }
    ...
    const flash = hunter ? (w.foxFlash.ready ? "  ·  [R] вспышка готова" : "  ·  вспышка " + Math.ceil(w.foxFlash.cooldown) + "с") : "";
    this.prompt.setIcon(act?.icon ?? (hunter ? "ui/icon_flashlight" : null)).setText((act?.text ?? "") + flash || "");
