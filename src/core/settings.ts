...
import { isKitId, type KitId } from "../data/characters";
...
export interface Settings {
  ...
  quality: Quality;
  /** Last pick on the select screen and in the lobby: run or be the villain, and its kit (null: random). */
  role: Role;
  kit: KitId | null;
}

const STORAGE_KEY = "assylum.settings";
const DEFAULTS: Settings = { musicVolume: 0.5, sfxVolume: 0.85, ambienceVolume: 0.7, difficulty: "normal", showFps: true, quality: "high", role: "runner", kit: null };
...
function load(): Settings {
  try {
    ...
    return {
      ...
      role: raw.role === "hunter" ? "hunter" : "runner",
      kit: isKitId(raw.kit) ? raw.kit : null,
    };
  } catch {
    return { ...DEFAULTS };
  }
