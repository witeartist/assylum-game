// Contact grabs, decided by the authority (solo game or host) for every hunter–runner pair.
...
export function updateCatches(world: World): void {
  ...
  for (const hunter of world.threats()) {
    ...
    for (const runner of world.runners()) {
      ...
      if (dist(hunter.authPos, runner.authPos) < radius) world.round.grab(runner, hunter);
    }
  }
}
