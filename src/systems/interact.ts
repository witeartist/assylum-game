...
export class Interact {
  ...
  current(): Interaction | null {
    ...
    const door = gate >= 0 ? { icon: null, text: w.gates.gates[gate].open ? "E — закрыть дверь" : "E — открыть дверь", run: () => w.gates.toggle(gate, a), target: w.gates.gates[gate].sprite } : null;
    if (a.role === "hunter") {
      ...
      if (spot && !w.hiding.checking(spot)) {
        return { icon: "ui/icon_hide", text: spot.kind === "bed" ? "E — заглянуть под кровать" : "E — открыть шкафчик", run: () => w.hiding.requestCheck(spot, a), target: spot.sprite };
      }
      return null;
    }
